...
class TestDeleteExisting:
    def test_delete_existing_clip(self, cli_config: Path, run_sdk_cli) -> None:
        ...
        assert results[0]["data"]["folder"] == str(folder)
        assert results[0]["data"]["message"] == "Clip deleted"
    ...
    def test_delete_emits_jsonl_result_with_stage(self, cli_config: Path, run_sdk_cli) -> None:
        """Delete result includes stage='delete'."""
        cid = _insert_clip(cli_config / "clips.db")
        code, envelopes = run_sdk_cli(["delete", str(cid)])
        results = [e for e in envelopes if e["type"] == "result"]
        assert results[0]["data"]["stage"] == "delete"


class TestDeleteNonexistent:
    def test_delete_nonexistent_clip(self, cli_config: Path, run_sdk_cli) -> None:
        """delete 999 with non-existent ID → error with NOT_FOUND + exit 1."""
        code, envelopes = run_sdk_cli(["delete", "999"])
...
class TestDeleteFolderCleanup:
    def test_folder_removed_on_delete(self, cli_config: Path, run_sdk_cli) -> None:
        ...
        run_sdk_cli(["delete", str(cid)])

        # Folder should be gone
        assert not folder.exists()

    def test_folder_missing_is_nonfatal(self, cli_config: Path, run_sdk_cli) -> None:
    ...
    def test_folder_cleanup_failure_emits_warning(self, cli_config: Path, monkeypatch: pytest.MonkeyPatch, run_sdk_cli) -> None:
        ...
        warnings = [e for e in envelopes if e["type"] == "warning"]
        results = [e for e in envelopes if e["type"] == "result"]
    ...
    def test_empty_folder_path_skips_cleanup(self, cli_config: Path, run_sdk_cli) -> None:
...
class TestDeleteIsolation:
    def test_delete_does_not_affect_other_clips(self, cli_config: Path, run_sdk_cli) -> None:
